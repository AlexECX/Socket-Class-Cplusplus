...
int BaseSocket::sendFile(string FilePath)
{
    ...
    else
    {
        ...
        while (SentBytes < File_size)
        {
            ...
            nRet = sendStr(string(File_buffer.get(), Msg_size));
            if (nRet > 0)
            {
                SentBytes += Msg_size;
            }
...
int BaseSocket::sendStr(const string &message)
{
    int nRet = 0;
...
int BaseSocket::recvStr(string &message)
{
    int nRet;
    unsigned Msg_size = 0;
    unsigned offset = 0;
...
void BaseSocket::sendStr_Ex(const std::string &message)
{
    if (this->sendStr(message) < 0)
    {
        throw SocketException(this->socket_err, TRACEBACK);
    }
}

void BaseSocket::recvStr_Ex(std::string &message)
{
    if (this->recvStr(message) <= 0)
    {
        throw SocketException(this->socket_err, TRACEBACK);
    }
}

std::string BaseSocket::recvStr_Ex()
{
    string message = "";
    this->recvStr_Ex(message);
    return message;
}
